...
gxp.ScaleOverlay = Ext.extend(Ext.Panel, {
    ...
    handleZoomEnd: function() {
        ...
        if (scale.length > 0) {
            scale = scale.items[0];
            this.zoomSelector.setValue("1 : " + (Math.round(parseFloat(scale.data.scale) / 5) * 5));
        } else {
    ...
    addScaleCombo: function() {
        ...
        this.zoomSelector = new Ext.form.ComboBox({
            ...
            tpl: '<tpl for="."><div class="x-combo-list-item">1 : {[(Math.round(parseFloat(values.scale) / 5) * 5)]}</div></tpl>',
            editable: false,
            triggerAction: 'all',
            mode: 'local',
